...
''' 
    class for crawling followers that user owning and user relationship by user.txt 
    where store the user list
    and store the result to file user_follow.txt
'''
class UserFollowedSpider(scrapy.spiders.Spider):
    name = "user_followed"
    allowed_domains = ["github.com"]
    src_table = 'user'
    src_key = ['login']
    des_table = 'user_user'
    des_key = ['followed','following']
    des_val = {}
    url_rela_name = 'followers'
    rela_key = 'login'
    rela_num = 0
    cur_item_ind = 0
    cur_item = ''
